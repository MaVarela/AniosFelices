...
{
    public partial class frmConsultarEstadoGeneral : Form
    {
        IRepositorioLibroDeGuardias repositorioLibroDeGuardias = new RepositorioLibroDeGuardias();

        public frmConsultarEstadoGeneral()
        {
            InitializeComponent();
        }

        private void frmConsultarEstadoGeneral_Load(object sender, EventArgs e)
        {
            cargar();
        }
        ...
        private void cargar()
        {
            try
            {
                ...
                consultaActiva.Consulta = "ConsultarEstado";
                cargarCmbFiltrar();
            ...
            try
            {
                ...
                else
                {
                    if (cmbFiltrar.SelectedItem.ToString() == "Paciente - Fecha")
                    {
                        ...
                        this.dgvEstado.DataSource = mapper.llenarListaLibroDeGuardiasDTO((List<LibroDeGuardias>)repositorioLibroDeGuardias.BuscarRegistros(dtpFecha.Value.Date, null, null, cmbTurno.Text + ", Estado General"));
                }
                this.dgvEstado.Columns[6].Visible = false;
                this.dgvEstado.Columns[7].Visible = false;
            }
